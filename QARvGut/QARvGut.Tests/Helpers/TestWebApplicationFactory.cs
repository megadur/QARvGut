...
namespace QARvGut.Tests.Helpers
{
    public class TestWebApplicationFactory<TProgram> : WebApplicationFactory<TProgram> where TProgram : class
    {
        ...
        {
            // Create a completely custom web host that doesn't use Program.cs
            ...
            builder.UseKestrel()
                ...
                .ConfigureAppConfiguration((context, config) =>
                {
                    config.AddInMemoryCollection(new Dictionary<string, string?>
                    {
                        ...
                        { "TestDatabaseName", _databaseName } // Pass the database name to startup
                    });
                })
        ...
        public async Task InitializeAsync()
        {
            ...
            {
                var logger = scopedServices.GetService<ILogger<TestWebApplicationFactory<TProgram>>>();
        ...
        private async Task SeedData(ApplicationDbContext context, UserManager<ApplicationUser> userManager, RoleManager<ApplicationRole> roleManager)
        {
            ...
            if (await userManager.FindByNameAsync("testuser1") == null)
            {
                ...
                var result = await userManager.CreateAsync(testUser1, "P@ssword123");
                if (result.Succeeded)
                {
            ...
            if (await userManager.FindByNameAsync("testuser2") == null)
            {
                ...
                var result = await userManager.CreateAsync(testUser2, "P@ssword123");
                if (result.Succeeded)
                {
